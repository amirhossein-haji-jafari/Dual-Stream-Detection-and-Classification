...
def non_max_suppression(prediction, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
    # The input `prediction` is a single 2D tensor for one image, not a list.
    # So we wrap it in a list to use the existing loop structure.
    prediction = [prediction]
    output = [None for _ in range(len(prediction))]

    for i, image_pred in enumerate(prediction):
        # image_pred is now the 2D tensor of detections
        
        # We need to handle the case where image_pred might be empty
        if image_pred is None or not image_pred.size(0):
            continue
        ...
        # Handle case where no boxes pass the confidence threshold
