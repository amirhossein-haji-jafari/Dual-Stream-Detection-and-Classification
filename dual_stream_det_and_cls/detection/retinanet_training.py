...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions, anchors, annotations, alpha = 0.25, gamma = 5.0, cuda = True): # Try gamma=1.5
        ...
        for j in range(batch_size):
            ...
            if positive_indices.sum() > 0:
                ...
                # smooth l1 loss. beta = 1/9
                # regression_diff is the element-wise absolute difference |prediction - target|
                regression_diff = torch.abs(targets - regression[positive_indices, :])
                regression_loss = torch.where(
                    torch.le(regression_diff, 1.0 / 9.0), # if |diff| <= 1/9
                    0.5 * 9.0 * torch.pow(regression_diff, 2), # then 0.5 * 9 * diff^2
                    regression_diff - 0.5 / 9.0 # else |diff| - 0.5/9
                )
